...
let bgImg = new Image()
bgImg.src = "public/dirt-image.png"
...
function drawHoe(event) {
  ...
  ctx.beginPath()
  ctx.arc(
    mouseX - getRandom(5, 25),
    mouseY - getRandom(5, 25),
    getRandom(8, 18) * baseSize,
    0,
    2 * Math.PI
  )
  ctx.fill()
...
function drawPencil(event) {
  ...
  points.forEach((point) => {
    ...
    for (let i = 0; i < 4 * speed; i++) {
      ...
      ctx.fillRect(
        point.x + rand() * spread + sliderValue,
        point.y + rand() * spread + sliderValue,
        sliderValue * 0.008,
        sliderValue * 0.01
      )
    }
...
document.querySelectorAll(".stamps").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    removeAllBrushEventListeners()
    //isDrawing = true;
    currentDrawFunction = function (event) {
      drawStamp(event, emoji)
    }
...
function drawStamp(event, emoji) {
  ...
  ctx.fillText(
    emoji,
    event.clientX * pixelRatio - sliderValue / 2,
    event.clientY * pixelRatio + sliderValue / 2
  )
}
...
document.querySelectorAll(".color-picker-label").forEach((label) => {
  ...
  label.style.backgroundImage = 'URL("public/rainbow.png")'
